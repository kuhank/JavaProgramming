...
public class RetrieveServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try
		{
			RetrieveServlet.initialProcess();
		
       // PreparedStatement st = con.prepareStatement("select * from personal where empid=?"); 
...
if (rs==null)
	out.println("<html><body><b>No Record"
            + "</b>");
else
{     while(rs.next())
        	 out.println("<html><body><b>"+rs.getString(2)+rs.getString(3)+ "</b><br>");
}
